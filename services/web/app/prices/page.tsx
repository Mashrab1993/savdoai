...
const GROUP_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  VIP:      { bg: "bg-amber-500/10 dark:bg-amber-950/20",  text: "text-amber-700 dark:text-amber-300 dark:text-yellow-300",  border: "border-amber-500/30 dark:border-yellow-800" },
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: locale === "uz" ? "Mijozlar"        : "Клиенты",                            value: String(clients.length), icon: Users,   color: "text-emerald-500",  bg: "bg-emerald-500/15 dark:bg-emerald-950/20" },
            { label: locale === "uz" ? "O'rtacha chegirma" : "Средняя скидка",                   value: `${avgDiscount}%`,     icon: Percent, color: "text-amber-500", bg: "bg-amber-500/15 dark:bg-amber-950/20" },
          ].map(s => (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-right text-emerald-600 dark:text-emerald-400 dark:text-emerald-400">{locale === "uz" ? "Chegirmali narx" : "Цена со скидкой"}</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {filteredProducts.map(p => (
                          <TableRow key={p.id} className="border-b border-border hover:bg-secondary/50">
                            ...
                            <TableCell className="text-right text-sm font-semibold text-emerald-600 dark:text-emerald-400 dark:text-emerald-400">
