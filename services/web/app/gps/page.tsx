  ...
  return (
    <AdminLayout>
      <div className="max-w-[1300px] mx-auto space-y-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">GPS</h1>
          <p className="text-base text-slate-500 mt-1">Agent kuzatuvi{!isAuthenticated && <span className="ml-2 text-amber-600 text-xs">⚠ Login kerak</span>}</p>
        </div>
        <Card className="p-6 bg-blue-50 border-blue-200">
          <div className="flex items-start gap-3">
            <MapPin className="w-7 h-7 text-blue-600 flex-shrink-0" />
            <div>
              <h3 className="font-semibold text-blue-900">GPS moduli</h3>
              <p className="text-sm text-blue-800 mt-1">Agent kuzatuvi — modul rivojlantirilmoqda. Real ma'lumotlar mavjud: {klients.length} ta klient.</p>
            </div>
          </div>
        </Card>
        {klients.length > 0 && (
