        ...
        {tabs.map((t: any) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`px-4 py-2.5 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${
              tab === t.id ? "bg-primary text-primary-foreground shadow-lg shadow-primary/20" : "bg-muted dark:bg-muted text-muted-foreground dark:text-muted-foreground"
            }`}>{t.label}</button>
        ))}
      </div>
      ...
      {!loading && tab === "abc" && abc && (
        <>
          ...
          <div className="grid grid-cols-4 gap-1 mb-6">
            <div className="p-2 text-center text-xs font-bold text-muted-foreground" />
            {["X (Barqaror)", "Y (Mavsumiy)", "Z (Tartibsiz)"].map((x: string) => (
              <div key={x} className="p-2 text-center text-xs font-bold text-muted-foreground bg-muted/50 dark:bg-muted rounded">{x}</div>
            ))}
            {["A (Top 80%)", "B (O'rta 15%)", "C (Quyi 5%)"].map((a: string, ai: number) => (
              <>
                <div key={a} className="p-2 text-xs font-bold text-muted-foreground bg-muted/50 dark:bg-muted rounded flex items-center">{a}</div>
                {["X", "Y", "Z"].map((x: string) => {
          ...
          <Card>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-muted/50 dark:bg-muted text-left text-xs">
                ...
                <tbody>
                  {(abc.tovarlar || []).slice(0, 30).map((t: any, i: number) => (
                    <tr key={i} className="border-t border-border/60 dark:border-border">
                      ...
                      <td className="px-4 py-2.5 text-xs text-muted-foreground">{t.cv}</td>
                      <td className="px-4 py-2.5 text-xs text-muted-foreground max-w-48 truncate">{t.tavsiya}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">{k.telefon}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="text-[10px] text-muted-foreground">risk skor</div>
                  </div>
                </div>
                <div className="mt-2 text-xs text-muted-foreground dark:text-muted-foreground">{k.harakat}</div>
            ...
            {(reorder || []).map((t: any, i: number) => (
              <div key={i} className="flex items-center justify-between px-4 py-3">
                ...
                <div className="text-right">
                  ...
                  <div className="text-[10px] text-muted-foreground">{t.kunlar_qoldi} kun qoldi</div>
                </div>
              </div>
            ))}
            {(!reorder || reorder.length === 0) && (
              <div className="text-center py-12 text-muted-foreground">Hozircha qayta buyurtma kerak emas</div>
            )}
          </div>
        </Card>
      )}
