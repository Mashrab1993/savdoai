...
export default function InvoicesPage() {
  ...
  const kpiCards = [
    ...
    { label: t.tolangan, value: fmt(stats.bugun_tolangan), icon: CheckCircle2, color: "text-green-600 dark:text-green-400",        bg: "bg-green-100 dark:bg-green-900/20" },
    { label: t.qarzCol,  value: fmt(stats.bugun_qarz),     icon: AlertTriangle,color: "text-destructive",                          bg: "bg-red-100 dark:bg-red-900/20" },
    { label: t.soni,     value: String(stats.bugun_soni),   icon: Clock,        color: "text-yellow-600 dark:text-yellow-400",      bg: "bg-yellow-100 dark:bg-yellow-900/20" },
  ]
              ...
              <TableBody>
                {items.length === 0 ? (
                  ...
                  <TableRow key={s.id} className="hover:bg-secondary/50 transition-colors cursor-pointer" onClick={() => openDetail(s.id)}>
                    ...
                    <TableCell className="text-right text-sm text-green-600 dark:text-green-400">{fmt(s.tolangan ?? 0)}</TableCell>
