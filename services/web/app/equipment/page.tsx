    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              <Refrigerator className="w-7 h-7 text-emerald-600" />
              Uskunalar (Inventar)
            </h1>
            <p className="text-sm text-muted-foreground mt-1">Mijozlarga berilgan uskunalar va ularning holati</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {TYPES.map(t => (
            <div key={t.key} className="bg-white dark:bg-gray-900 rounded-xl border p-4 text-center hover:shadow-md transition">
              ...
              {equipment.length === 0 ? (
                ...
                <TableRow key={i}>
                  ...
                  <TableCell className="text-center">
                    <Badge className={e.holat === "yaxshi" ? "bg-emerald-100 text-emerald-800" : e.holat === "buzilgan" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}>
                      {e.holat || "Yaxshi"}
                    </Badge>
                  </TableCell>
                  <TableCell>
