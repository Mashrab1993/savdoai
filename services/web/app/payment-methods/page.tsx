    ...
    <AdminLayout>
      <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {methods.map(m => (
            <div key={m.id} className="bg-white dark:bg-gray-900 rounded-xl border p-4 hover:shadow-md transition">
              <div className="flex items-start justify-between mb-3">
                <m.icon className="w-8 h-8 text-emerald-600" />
                <Badge className={m.faol ? "bg-emerald-100 text-emerald-800" : "bg-gray-100 text-gray-600"}>{m.faol ? "Faol" : "Nofaol"}</Badge>
              </div>
