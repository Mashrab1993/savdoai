...
import {
  Receipt, Search, Plus, Check, X, Clock,
...
const emptyForm = { title: "", category: "Transport", amount: 0, requestedBy: "Alisher Ergashev", notes: "" }
...
export default function ExpensesPage() {
  ...
  function approve(id: string) {
    setExpenses(prev => prev.map(e =>
      e.id === id ? { ...e, status: "approved" as const, approvedBy: "Alisher Ergashev" } : e
    ))
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                    ...
                    >
                      <TableCell>
                        <div>
                          ...
                          {exp.notes && (
                            <p className="text-xs text-muted-foreground mt-0.5 truncate max-w-[180px]">{exp.notes}</p>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-sm text-muted-foreground">{exp.requestedBy}</TableCell>
                      ...
                      <TableCell className="text-right">
                        {exp.status === "pending" ? (
                          ...
                          <span className="text-xs text-muted-foreground">
                            {exp.approvedBy ? exp.approvedBy.split(" ")[0] : "—"}
                          </span>
                        )}
                      </TableCell>
                    </TableRow>
          ...
          <div className="space-y-4">
            <div className="bg-card border border-border rounded-xl p-5">
              <h3 className="font-semibold text-sm text-foreground mb-1">Kategoriya bo'yicha</h3>
              <p className="text-xs text-muted-foreground mb-4">Xarajatlar taqsimoti</p>
                  ...
                  {expenses.filter(e => e.status === "pending").map(exp => (
                    <div key={exp.id} className="flex items-center justify-between gap-2 py-2 border-b border-border last:border-0">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{exp.requestedBy.split(" ")[0]} · {fmt(exp.amount)}</p>
                      </div>
                      <div className="flex gap-1 shrink-0">
                        <Button size="icon" variant="ghost" className="h-6 w-6 text-green-600 hover:bg-green-100 dark:hover:bg-green-900/30" onClick={() => approve(exp.id)}>
                          <Check className="w-3 h-3" />
                        </Button>
                        <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive hover:bg-red-100 dark:hover:bg-red-900/30" onClick={() => reject(exp.id)}>
                          <X className="w-3 h-3" />
                        </Button>
                      </div>
                    </div>
