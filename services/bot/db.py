...
def D(v: Any) -> Decimal:
    """Decimal conversion — shared/utils/hisob.py dan import qilingan nusxa.
    Bot db.py da lokal saqlangan (circular import oldini olish).
    Mantiq: hisob.py bilan bir xil."""
    ...
    async with _P().acquire() as c:
        ...
        await c.execute("""
            ...
            ON CONFLICT (id) DO UPDATE SET
                ism = COALESCE(NULLIF(users.ism, ''), EXCLUDED.ism),
...
async def sotuv_saqlash(uid: int, data: dict) -> dict:
    ...
    async with _P().acquire() as c:
        ...
        # ── Loyalty: avtomatik ball qo'shish ──
        if klient_id and jami > 0:
            try:
                from shared.services.loyalty import klient_ball_qoshish
                async with _P().acquire() as lc:
                    loyalty = await klient_ball_qoshish(
                        lc, uid, klient_id, float(jami), sess_id,
                        f"Sotuv #{sess_id}"
                    )
                    if loyalty.get("ball", 0) > 0:
                        log.info("⭐ Loyalty: +%d ball → %s (jami: %d)",
                                 loyalty["ball"], klient_ismi, loyalty.get("jami_ball", 0))
            except Exception as _le:
                log.debug("Loyalty auto: %s", _le)
        ...
        # ── Post-sale ogohlar — kam qoldiq, loyalty ball ──
        ogohlar = []
        if klient_id and jami > 0:
            try:
                ...
                if 'loyalty' in dir() and loyalty.get("ball", 0) > 0:
                    ...
                    loyalty_msg = (
                        ...
                        f"({d.get('nomi','')}) {d.get('emoji','')}"
                    )
                    ogohlar.append(loyalty_msg)
            except Exception:
                pass

        try:
            async with _P().acquire() as oc:
                kam = await oc.fetch("""
                    SELECT nomi, qoldiq, min_qoldiq FROM tovarlar
                    WHERE user_id=$1 AND min_qoldiq > 0 AND qoldiq <= min_qoldiq
                    AND qoldiq >= 0 ORDER BY qoldiq ASC LIMIT 5
                """, uid)
                for r in kam:
                    q = float(r["qoldiq"])
                    if q <= 0:
                        ogohlar.append(f"🚨 {r['nomi']}: TUGADI!")
                    else:
                        ogohlar.append(f"⚠️ {r['nomi']}: {q:.0f} qoldi")
        except Exception:
            pass
