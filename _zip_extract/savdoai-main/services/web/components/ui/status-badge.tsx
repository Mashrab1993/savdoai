...
// Maps StatusVariant to its i18n key in translations.status
const statusKeyMap: Record<StatusVariant, keyof typeof translations.status> = {
  active:        "active",
  inactive:      "inactive",
  prospect:      "prospect",
  "in-stock":    "inStock",
...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  ...
  const label = key && translations.status[key] ? translations.status[key][locale] : status
  return (
    <span className={cn("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold", variantMap[status] ?? "bg-secondary text-muted-foreground", className)}>
      {label}
    </span>
  )
}
