...
async def lifespan(app: FastAPI):
    ...
    if not dsn:
        log.critical("❌ DATABASE_URL sozlanmagan! API ishga tushmaydi.")
        sys.exit(1)

    if not _JWT_SECRET_RAW:
        log.critical("❌ JWT_SECRET sozlanmagan! API ishga tushmaydi.")
        sys.exit(1)
...
async def sotuv_saqlash(data: SotuvSo_rov, request: Request, uid: int = Depends(get_uid)):
    ...
    if klient_id and jami > 0:
        try:
            ...
            loyalty = await klient_ball_qoshish(c, uid, klient_id, jami, sess_id)
...
async def export_file_yuklab(task_id: str, token: Optional[str] = None, uid: int = Depends(get_uid)):
    """
...
@app.get("/api/v1/ombor/prognoz", tags=["Tovarlar"])
async def ombor_prognoz_api(kunlar: int = 30, limit: int = 50,
                             uid: int = Depends(get_uid)):
    """
    Tovar tugash prognozi — oxirgi N kun sotuv tezligi asosida.

    Har bir tovar uchun:
    - kunlik_sotuv: O'rtacha kunlik sotuv
    - qolgan_kun: Qoldiq necha kunga yetadi
    - holat: tugagan / kam / xavfli / ogohlantirish / yaxshi
    - buyurtma_kerak: True/False
    - buyurtma_miqdor: 2 haftalik zaxira uchun kerakli miqdor
    """
