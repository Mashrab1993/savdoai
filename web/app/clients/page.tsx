...
export default function ClientsPage() {
  ...
  const displayClients = clients.length > 0 ? clients : localClients
  const setClients = (fn: (prev: Client[]) => Client[]) => setLocalClients(fn(displayClients))
  ...
  async function handleSave() {
    ...
    if (refetch) {
      const res = await createClient({ ism: form.name!, telefon: form.phone || undefined, manzil: form.company || undefined })
      if ("error" in res) {
  ...
  const totalRevenue = displayClients.reduce((s, c) => s + c.totalPurchases, 0)
  const totalDebt = displayClients.reduce((s, c) => s + c.totalDebt, 0)

  if (loading && clients.length === 0) {
    return (
      <AdminLayout title={L.title[locale]}>
        <div className="flex items-center justify-center py-20">
          <p className="text-sm text-muted-foreground">{locale === "uz" ? "Yuklanmoqda…" : "Загрузка…"}</p>
        </div>
      </AdminLayout>
    )
  }
