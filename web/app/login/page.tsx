...
const HAS_API = typeof process.env.NEXT_PUBLIC_API_URL === "string" && process.env.NEXT_PUBLIC_API_URL.length > 0
...
export default function LoginPage() {
  ...
  async function submitTelegramAuth() {
    const uid = Math.floor(Number(telegramUserId))
    if (!uid || uid < 1) {
      setTelegramError(locale === "uz" ? "User ID raqam bo'lishi kerak" : "User ID должен быть числом")
      return
    }
    if (!telegramHash.trim()) {
      setTelegramError(locale === "uz" ? "Hash kiritish shart" : "Введите hash")
      return
    }
    setTelegramLoading(true)
    setTelegramError(null)
    const out = await postTelegramAuth({
      user_id: uid,
      ism: "",
      hash: telegramHash.trim(),
    })
    setTelegramLoading(false)
    if ("error" in out) {
      setTelegramError(out.error.message)
      return
    }
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        ...
        <div className="w-full max-w-sm space-y-7">
          ...
          <div className="space-y-3">
            ...
            >
              {telegramLoading && !telegramModalOpen ? (
            ...
            <p className="text-center text-xs text-muted-foreground">
              {HAS_API
                ? (locale === "uz"
                  ? "Backend ulangan. User ID va hash kiriting yoki Mini App orqali kiring."
                  : "Backend подключен. Введите User ID и hash или войдите через Mini App.")
                : L.telegramNote[locale]
              }
            </p>
          </div>
          ...
          <Dialog open={telegramModalOpen} onOpenChange={setTelegramModalOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>
                  {locale === "uz" ? "Telegram tasdiqlash" : "Подтверждение Telegram"}
                </DialogTitle>
              </DialogHeader>
              <p className="text-sm text-muted-foreground">
                {locale === "uz"
                  ? "Bot yoki Mini App dan olingan User ID va auth hash kiriting."
                  : "Введите User ID и auth hash из бота или Mini App."}
              </p>
              <div className="space-y-3">
                <div>
                  <Label htmlFor="tg-uid">User ID</Label>
                  <Input
                    id="tg-uid"
                    type="text"
                    inputMode="numeric"
                ...
                <div>
                  <Label htmlFor="tg-hash">Auth hash</Label>
                  <Input
                    id="tg-hash"
                    type="text"
                    placeholder="..."
              ...
              <DialogFooter>
                ...
                <Button onClick={submitTelegramAuth} disabled={telegramLoading}>
